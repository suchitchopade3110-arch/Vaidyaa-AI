...
import {
  ...
  PipelineStepper, StatusBadge, ResultHeader, ConfidenceMeter, UncertaintyBanner,
...
const GRADCAM_SEEDS_BY_TYPE: Record<string, any[]> = {
...
function percentNumber(value: any, fallback: number) {
...
function regionIntensity(region: any, fallback = 0.5) {
...
function severityFromIntensity(intensity: number) {
  if (intensity >= 0.76) return 'High';
  if (intensity >= 0.48) return 'Moderate';
  return 'Low';
}

function normalizeGradcamRegions({ result, type }: { result: any, type: string }) {
...
function gradcamColor(intensity: number) {
...
function drawSyntheticXray(ctx: CanvasRenderingContext2D, w: number, h: number, type: string) {
...
function drawGradcamOverlay(ctx: CanvasRenderingContext2D, w: number, h: number, regions: any[], opacity: number, selectedId: string) {
...
function WHOChecklistPanel({ citations = [] }: { citations?: any[] }) {
  ...
  return (
    <div style={{ border: '1px solid var(--border)', borderRadius: '8px', background: 'var(--bg-surface)', padding: '12px', minHeight: 0 }}>
      ...
      <div style={{ marginTop: '12px', paddingTop: '10px', borderTop: '1px solid var(--border)' }}>
        ...
        {(whoCitations.length ? whoCitations : [{ title: 'WHO imaging standards pending retrieval', source: 'medical_evidence_week1_clean', snippet: 'Run the WHO ingestion script to enable direct RAG citations.' }]).map((citation: any, index: number) => (
...
function CytopathologyStandardsPanel({ result }: { result: any }) {
  ...
  const criteriaRow = (item: any, active: boolean, onClick: () => void) => (
    <button
      key={item.id}
      type="button"
      onClick={onClick}
...
function GradCAMViewer({ type, result, file }: { type: string, result: any, file: File | null }) {
...
function YoloDetectionPanel({ result }: { result: any }) {
  ...
  return (
    <Card>
      ...
      <div style={{ marginTop: '10px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
        {detections.length ? detections.map((det: any, index: number) => (
      ...
      const poll = async () => {
        try {
          ...
          setResult({
            ...
            citations: (payload.citations || payload.sources || []).map((c: any) => ({
            ...
            <div style={{ flex: '1', minWidth: '260px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <Card>
                ...
                {result.findings?.length > 0 ? (
                  result.findings.map((f: any, i: number) => <FindingRow key={i} finding={f} />)
                  ...
                  <div style={{ padding: '12px', background: 'var(--bg-elevated)', borderRadius: '7px', border: '1px solid var(--border)' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                      {(result.classification.all_findings || []).map((finding: any) => (
