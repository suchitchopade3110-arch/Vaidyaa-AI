// Ported from ui/image-analysis.jsx — Image Analysis page.
//
// Real behavior changes from the original:
//  1. Both fetch() calls now go through apiFetch() (bearer token
//     attached — POST/GET /api/v1/analyze/image/* requires it per
//     SEC-01/SEC-02).
//  2. Fixed the same pre-existing `window.ReportQRWidget &&` bug noted
//     in ReportAnalyzer.jsx — replaced with a check on serverAnalysisId.
import React, { useState, useRef, useEffect, useMemo } from "react";
import {
  Button, Card, CitationList, ConfidenceMeter, FileUploadZone,
  MedicalDisclaimer, PatientIdField, PipelineStepper, ResultHeader,
  SectionLabel, SegmentedControl, StatusBadge, UncertaintyBanner,
...
export default function ImageAnalysis() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const submitRes = await apiFetch(`/analyze/image/${analysisType}`, {
        method: 'POST',
        body: formData,
      });
      ...
      const poll = async () => {
        try {
          const pollRes = await apiFetch(`/analyze/image/${analysisId}`);
      ...
      {phase === 'done' && result && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div style={{ display: 'flex', gap: '16px', alignItems: 'stretch', flexWrap: 'wrap' }}>
            ...
            {serverAnalysisId && <ReportQRWidget reportId={serverAnalysisId} patientId={patientId} />}
          </div>
