// Ported from ui/claim-verifier.jsx — Claim Verifier page.
//
// Real behavior change from the original: both fetch() calls now go
// through apiFetch() (bearer token attached — POST /api/v1/verify/claim
// and GET /api/v1/verify/claim/{id} both require it per SEC-01/SEC-02;
// the GET additionally requires the caller to own the job).
import React, { useState, useRef } from "react";
import {
  ApiErrorAlert, Button, Card, ConfidenceMeter, EntityChip, MedicalDisclaimer,
  PatientIdField, PipelineStepper, ResultHeader, SectionLabel, StatusBadge,
  UncertaintyBanner,
...
export default function ClaimVerifier() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const submitRes = await apiFetch(`/verify/claim`, {
        method: 'POST',
      ...
      const poll = async () => {
        try {
          const pollRes = await apiFetch(`/verify/claim/${claimId}`);
