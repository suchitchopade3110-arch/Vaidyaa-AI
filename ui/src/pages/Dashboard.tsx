...
import { StatusBadge, MedicalDisclaimer } from '../components/Shared';

const API_BASE = `${window.location.origin}/api/v1`;

const PIPELINE_ICONS: Record<string, string> = { claim: '◎', report: '▤', image: '⬡' };
const PIPELINE_LABELS: Record<string, string> = { claim: 'Claim', report: 'Report', image: 'Image' };

// Map backend status values to frontend display statuses
function mapStatus(pipeline: string, statusVal?: string) {
...
function JobRow({ job, last, onNavigate }: { job: any, last: boolean, onNavigate: (p: string) => void }) {
  const [hov, setHov] = useState(false);
  const pageMap: Record<string, string> = { claim: 'claim', report: 'report', image: 'image' };
  return (
    <tr
      onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      style={{
