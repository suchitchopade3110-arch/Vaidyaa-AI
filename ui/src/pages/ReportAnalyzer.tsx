...
import { 
  SectionLabel, Card, Button, PatientIdField, MedicalDisclaimer, 
  PipelineStepper, StatusBadge, ResultHeader, ConfidenceMeter, EntityChip,
  FileUploadZone, SegmentedControl, CitationList 
} from '../components/Shared';

const API_BASE = `${window.location.origin}/api/v1`;

const REPORT_STEPS: Record<string, Array<{label: string, desc: string}>> = {
  lab: [
...
function ExplanationBlock({ brief, full }: { brief?: string, full?: string }) {
...
function DrugInteractionChips({ medications = [] }: { medications?: any[] }) {
  const interactions: Record<string, { severity: string, note: string }> = {
  ...
  const colors: Record<string, string> = { high: 'oklch(0.65 0.20 25)', medium: 'oklch(0.75 0.14 60)', low: 'oklch(0.46 0.19 145)' };
  return (
...
function DifferentialDxBlock({ ddx }: { ddx: any }) {
  const [open, setOpen] = useState(false);
  if (!ddx || ddx.verdict === 'insufficient_evidence') return null;
  const urgencyColor: Record<string, string> = { emergency: 'oklch(0.60 0.22 25)', urgent: 'oklch(0.65 0.20 25)', follow_up: 'oklch(0.75 0.14 60)', routine: 'oklch(0.46 0.19 145)' };
...
function PatientHistoryDiff({ comparison, history }: { comparison: any, history: any[] }) {
  const [open, setOpen] = useState(false);
  ...
  const directionColor = (value: string) => value === 'up' || value === 'worsened' ? 'oklch(0.65 0.20 25)' : value === 'down' || value === 'improved' ? 'oklch(0.46 0.19 145)' : 'var(--text-muted)';
  const directionIcon = (value: string) => value === 'up' || value === 'worsened' ? '▲' : value === 'down' || value === 'improved' ? '▼' : '→';
          ...
          {!!labDeltas?.length && (
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
                ...
                <tbody>
                  {labDeltas.slice(0, 8).map((item: any, index: number) => {
...
function BiasAuditDashboard() {
  ...
  const AccBar = ({ label, value, overall }: { label: string, value: number, overall: number }) => {
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              {['gender_metrics', 'age_group_metrics', 'region_metrics'].map((groupKey) => (
                <div key={groupKey}>
                  ...
                  {Object.entries(data[groupKey] || {}).map(([group, metrics]: [string, any]) => (
...
function LabTable({ rows, anomalyMap = {} }: { rows: any[], anomalyMap?: any }) {
  return (
    <div style={{ overflowX: 'auto' }}>
...
function ShapRow({ factor, score }: { factor: string, score: number }) {
...
function normalizeShap(payload: any) {
  ...
  return Object.entries(raw || {}).map(([factor, score]) => ({ factor, score: score as number }));
}

function processPayload(payload: any, reportType: string) {
  ...
  const labRows = Array.isArray(rawLabValues)
    ...
    : Object.entries(rawLabValues || {}).map(([key, value]: [string, any]) => ({
        name: key,
        test: key,
  ...
  return {
    ...
    citations: (payload.sources || payload.source_citations || []).map((item: any) => ({
...
export default function ReportAnalyzer() {
  ...
  const handleFile = (item: File) => { if (validateFile(item)) setFile(item); };

  const finishResult = (payload: any) => {
  ...
  const fallbackPoll = (reportId: string) => {
    const poll = async () => {
      try {
  ...
  const steps = REPORT_STEPS[reportType] || [];
  const anomalyMap: any = {};
  result?.anomalies?.forEach((item: any) => {
        ...
        <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
          ...
          <Card style={{ flex: '2', minWidth: '260px' }}>
            ...
            <div style={{ marginTop: '12px', padding: '12px', background: 'var(--bg-elevated)', borderRadius: '7px', border: '1px solid var(--border)', fontSize: '13px' }}>
              ...
              <div style={{ marginTop: '4px', fontSize: '12px', color: 'var(--text-muted)' }}>{reportType} · {((file?.size || 0) / 1024).toFixed(1)} KB</div>
            </div>
              ...
              <div style={{ marginTop: '16px', display: 'flex', gap: '14px', flexWrap: 'wrap' }}>
                <div style={{ flex: 1, minWidth: '160px', padding: '14px', background: 'var(--bg-elevated)', borderRadius: '8px', border: '1px solid var(--border)' }}>
                  ...
                  {result.ensemble?.model_scores && (
                    <div style={{ marginTop: '10px', fontSize: '10px', color: 'var(--text-muted)' }}>
                      {Object.entries(result.ensemble.model_scores).filter(([, value]) => value != null).map(([model, value]: [string, any]) => <div key={model}>▸ {model}: {Math.round(value)}</div>)}
                ...
                <div style={{ flex: 2, minWidth: '200px' }}>
                  ...
                  <div style={{ marginTop: '14px' }}>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>{result.conditions?.map((item: any, index: number) => <EntityChip key={index} label={item} type="condition" />)}</div>
                    <div style={{ marginTop: '10px', display: 'flex', flexWrap: 'wrap', gap: '6px' }}>{result.medications?.map((item: any, index: number) => <EntityChip key={index} label={item?.name || item} type="medication" />)}</div>
          ...
          <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap' }}>
            <Card style={{ flex: 1, minWidth: '240px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {result.anomalies?.map((item: any, index: number) => (
            ...
            <Card style={{ flex: 1, minWidth: '240px' }}>
              ...
              {result.shap?.length > 0 ? result.shap.map((item: any, index: number) => <ShapRow key={index} factor={item.factor} score={item.score} />) : <p style={{ color: 'var(--text-muted)', fontSize: '13px', fontStyle: 'italic', marginTop: '10px' }}>SHAP not applicable for this panel type.</p>}
            </Card>
          </div>
