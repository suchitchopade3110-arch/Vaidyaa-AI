...
import { 
  SectionLabel, Card, Button, PatientIdField, ApiErrorAlert, MedicalDisclaimer, 
  PipelineStepper, StatusBadge, ResultHeader, ConfidenceMeter, EntityChip, UncertaintyBanner 
...
function ShapBar({ factor, score }: { factor: string, score: number }) {
...
function PlainExplanation({ text, claim, verdict }: { text: string, claim: string, verdict: string }) {
  if (!text) return null;
...
function EvidenceCard({ citation, index }: { citation: any, index: number }) {
...
export default function ClaimVerifier() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const body: any = { claim_text: claim };
      ...
      const poll = async () => {
        try {
          ...
          const shapArr = Array.isArray(shapRaw)
            ? shapRaw.map((item: any) => ({ factor: item.feature || item.factor || String(item), score: item.shap || item.score || 0 }))
            : Object.entries(shapRaw).map(([k, v]) => ({ factor: k, score: v as number }));

          const entities = (raw.extracted_entities?.conditions || raw.entities || nested.entities || [])
            .map((e: any) => ({ label: e.label || e, type: e.type || 'condition' }));
          ...
          setResult({
            ...
            citations: sources.map((c: any, i: number) => {
              if (typeof c === 'string') {
            ...
            }).filter((c: any) => !`${c.title} ${c.source}`.toLowerCase().includes('keyword_fallback')),
          });
          setPhase('done');
        } catch (err) {
            ...
            <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap' }}>
              {result.entities.length > 0 && (
                <Card style={{ flex: 1, minWidth: '240px' }}>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                    {result.entities.map((e: any, i: number) => <EntityChip key={i} label={e.label} type={e.type} />)}
                  </div>
                </Card>
              )}
              {result.shap.length > 0 && (
                <Card style={{ flex: 1, minWidth: '240px' }}>
                  ...
                  {result.shap.map((s: any, i: number) => <ShapBar key={i} factor={s.factor} score={s.score} />)}
                </Card>
              )}
            </div>
          )}
          ...
          {result.citations.length > 0 && (
            <Card>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginTop: '4px' }}>
                {result.citations.map((c: any, i: number) => <EvidenceCard key={i} citation={c} index={i + 1} />)}
              </div>
            </Card>
          )}
