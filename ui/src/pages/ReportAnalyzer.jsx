// Ported from ui/report-analyzer.jsx — Report Analyzer.
//
// Real behavior changes from the original:
//  1. All three fetch() calls now go through apiFetch() (bearer token
//     attached — every /api/v1/analyze/report/* and /api/v1/admin/*
//     route requires it per SEC-01/SEC-02).
//  2. The WebSocket URL now carries ?token= (app/routes/websocket_
//     routes.py requires it since SEC-02) via wsUrl().
//  3. Fixed a pre-existing bug: `{window.ReportQRWidget && <ReportQRWidget
//     .../>}` always rendered nothing once this stopped being a global
//     (window.ReportQRWidget is never set in the ported app) — the QR
//     widget would have silently vanished. Replaced with a check on
//     serverJobId, which is what actually gates whether there's a report
//     to generate a QR for.
import React, { useState, useRef } from "react";
import {
  Button, Card, CitationList, ConfidenceMeter, EntityChip, FileUploadZone,
  MedicalDisclaimer, PatientIdField, PipelineStepper, ResultHeader,
  SectionLabel, SegmentedControl, StatusBadge,
} from "../components/shared.jsx";
import ReportQRWidget from "../components/ReportQrWidget.jsx";
import { apiFetch, wsUrl } from "../lib/api.js";
...
function BiasAuditDashboard() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const response = await apiFetch(`/admin/bias-audit`);
...
export default function ReportAnalyzer() {
  ...
  const fallbackPoll = (reportId) => {
    const poll = async () => {
      try {
        const response = await apiFetch(`/analyze/report/${reportId}`);
        const payload = await response.json();
  ...
  const connectWebSocket = (reportId) => {
    const ws = new WebSocket(wsUrl(`/ws/${reportId}`));
    wsRef.current = ws;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await apiFetch(`/analyze/report/${reportType}`, { method: 'POST', body: formData });
      if (!response.ok) {
      ...
      {phase === 'done' && result && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div style={{ display: 'flex', gap: '16px', alignItems: 'stretch', flexWrap: 'wrap' }}>
            ...
            {serverJobId && <ReportQRWidget reportId={serverJobId} patientId={patientId} />}
          </div>
