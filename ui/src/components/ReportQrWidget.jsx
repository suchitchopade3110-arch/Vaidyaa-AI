// Ported from ui/report-qr-widget.jsx — QR card for report/image result screens.
//
// GET /reports/{id}/qr (app/routes/qr_reports.py) takes an *optional*
// bearer token (patient-facing preview flow) rather than requiring one,
// so this deliberately still uses plain fetch(), not apiFetch — sending
// a token here wouldn't be wrong, but the endpoint doesn't need it and
// the original never sent one either.
import React, { useEffect, useState } from "react";

export default function ReportQRWidget({ reportId, patientId }) {
  ...
  useEffect(() => {
    ...
    fetch(`${window.location.origin}/reports/${encodeURIComponent(reportId)}/qr${params}`)
      .then((response) => {
